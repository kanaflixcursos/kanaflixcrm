...
begin
  ...
  if not exists (
    ...
    where id = target_pipeline_id
      and public.is_current_organization(organization_id)
  ) then
  ...
  if expected_count <> cardinality(ordered_stage_ids)
    or exists (
      select 1 from unnest(ordered_stage_ids) as stage_id
      where not exists (
        select 1 from public.pipeline_stages
        where id = stage_id and pipeline_id = target_pipeline_id
      )
    )
  then
  ...
  from unnest(ordered_stage_ids) with ordinality as ordered(id, position)
