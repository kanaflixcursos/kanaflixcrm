...
O repositório está hospedado em `kanaflixcursos/kanaflixcrm`. A aplicação fica no diretório `kanaflix-crm/`; o projeto da Vercel deve usar esse diretório como **Root Directory**. Produção deve usar um projeto Supabase separado do desenvolvimento, com variáveis configuradas diretamente na Vercel.
