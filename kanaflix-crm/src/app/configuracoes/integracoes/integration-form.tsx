...
export function IntegrationForm({ provider, title, description, externalId, active, placeholder }: Readonly<{ provider: "meta_pixel" | "google_tag_manager"; title: string; description: string; externalId?: string; active?: boolean; placeholder: string }>) {
  ...
  return (
    <form action={formAction} className="rounded-3xl border border-border bg-surface p-6 shadow-sm sm:p-8">
      ...
      <label className="mt-5 flex items-center gap-3 rounded-2xl bg-surface-muted px-4 py-3 text-sm"><input name="enabled" type="checkbox" defaultChecked={active} className="size-4 accent-[var(--brand)]" /><span>Ativar quando o carregamento técnico estiver conectado</span></label>
