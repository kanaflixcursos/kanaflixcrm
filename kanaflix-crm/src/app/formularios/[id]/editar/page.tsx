...
export default async function EditFormPage({ params }: Readonly<{ params: Promise<{ id: string }> }>) {
  ...
  const { data: form } = await supabase.from("lead_forms").select("id, name, slug, title, description, success_message, redirect_url, status, fields, campaign_name, default_tags, allowed_origins, turnstile_enabled").eq("id", id).maybeSingle();
  if (!form) notFound();
